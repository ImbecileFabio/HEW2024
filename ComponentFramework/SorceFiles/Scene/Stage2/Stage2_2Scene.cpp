...
constexpr int gearCounter_2_2 = 1;		// ギアの獲得数
constexpr int hammerCounter_2_2 = 3;	// 叩ける上限
//--------------------------------------------------
// コンストラクタ
//--------------------------------------------------
Stage2_2Scene::Stage2_2Scene(GameManager* _gameManager)
	:SceneBase(_gameManager, "Stage2_2")
{
	Init();
}
//--------------------------------------------------
...
{
	auto mapData = tile_map_manager_->LoadCSV("MapData/Stage2/Stage2_2.csv");
	...
	gearMaxCount_ = gearCounter_2_2;	// 定数を代入
	hammerMaxCount_ = hammerCounter_2_2;
...
void Stage2_2Scene::Update()
{
	...
	case Stage2_2Scene::Game:
		NumberChange();
		if (game_manager_->GetItemCount() == gearCounter_2_2)
		{
			stageState_ = Result;
